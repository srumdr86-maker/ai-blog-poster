...
# 키워드 입력 창
keywords = st.text_input("꼭 들어갔으면 하는 키워드 (선택사항)", placeholder="예: 제주도, 흑돼지 맛집, 내돈내산")
    ...
    if not api_key:
        ...
        with st.spinner("AI가 사진을 분석하고 글을 작성 중입니다... 잠시만 기다려주세요 ⏳"):
            try:
                ...
                # AI에게 내릴 명령(프롬프트) 작성
                prompt = f"""
                당신은 프로페셔널한 {platform} 인플루언서입니다.
                첨부된 사진을 자세히 분석하고, 다음 조건에 맞춰 포스팅 글을 작성해 주세요.
                
                - 플랫폼: {platform} (플랫폼 성격에 맞게 해시태그나 이모지 활용)
                - 글투: {tone}
                - 포함할 키워드: {keywords if keywords else '사진에 보이는 특징들'}
                
                서론, 본론, 결론이 자연스럽게 이어지도록 작성하고, 읽는 사람이 흥미를 느낄 수 있게 매력적으로 써주세요.
                """
